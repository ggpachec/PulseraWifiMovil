...
class BluetoothLib {
  ...
  /*Future<bool> configureDev() async {
    Completer<bool> completer = Completer<bool>();
    try {
      await currentDev.requestMtu(244);
      print("MTU SET a 244");

      List<BluetoothService> services = await currentDev.discoverServices();
      services.forEach((service) async {
        if (service.uuid.toString().toLowerCase().startsWith(
            '6e400001')) { // 6e400001-b5a3-f393-e0a9-e50e24dcca9e
          print(
              "SERVICIO MODO TECNICO ENCONTRADO: ${service
                  .uuid}  ->>  ${service
                  .remoteId}");
          ...
          for (BluetoothCharacteristic c in characteristics) {
            //print(c);
            if (c.uuid.toString().toLowerCase().startsWith(
                '6e400002')) { //write 6e400002-b5a3-f393-e0a9-e50e24dcca9e
              serialWrite = c;
              print("characteristic write MODO TECNICO ${c.uuid}");
            }

            if (c.uuid.toString().toLowerCase().startsWith(
                '6e400003')) { //read 6e400003-b5a3-f393-e0a9-e50e24dcca9e
              serialRead = c;
              print("characteristic read MODO TECNICO ${c.uuid}");
              ...
              /*c.onValueReceived.listen((event) {
                try {
                  ...
                  if (result.isNotEmpty) {
                    serialReadData = result;
                    print("onValueReceived MODO TECNICO $result");
                  }
                } on Exception catch (e) {
                  print("Exception onValueReceived MODO TECNICO - " + e.toString());
                }
              });*/
            }
          }
        ...
        if (service.uuid.toString().toLowerCase().startsWith(
            '4a4f0001')) { //4a4f0001-4156-4920-9a23-4e657454454c
          print(
              "SERVICIO MODO NETTEL ENCONTRADO ${service.uuid}  ->>  ${service
                  .remoteId}");

          var characteristics = service.characteristics;
          for (BluetoothCharacteristic d in characteristics) {
            //print(d);
            if (d.uuid.toString().toLowerCase().startsWith(
                '4a4f0002')) { //write 4a4f0002-4156-4920-9a23-4e657454454c
              nettelWrite = d;
              print("characteristic write MODO NETTEL ${d.uuid}");
            }

            if (d.uuid.toString().toLowerCase().startsWith(
                '4a4f0003')) { //read 4a4f0003-4156-4920-9a23-4e657454454c
              nettelRead = d;
              print("characteristic read  MODO NETTEL ${d.uuid}");

              await d.setNotifyValue(true);
              /*d.onValueReceived.listen((event) {
                try {
                  String result = utf8.decode(event);
                  result = result.trim();
                  if (result.isNotEmpty) {
                    nettelReadData = result;
                    print("onValueReceived MODO NETTEL $result");
                  }
                } on Exception catch (e) {
                  print("Exception onValueReceived MODO NETTEL - "  + e.toString());
                }
              });*/
            }
          }
    ...
    print("RESPUESTA de connectDev ${completer.future.val}");
    return completer.future;
  }*/
